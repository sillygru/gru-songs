...
class MediaKitPlayer extends AudioPlayerPlatform {
  ...
  Future<ConcatenatingInsertAllResponse> concatenatingInsertAll(
    ...
    for (final source in request.children) {
      ...
      // `add` appends to the last index (length - 1); moving from `length`
      // itself is out of bounds and silently no-ops in media_kit, which
      // leaves the inserted item at the end of the playlist instead of at
      // the requested index.
  ...
  Future<ConcatenatingMoveResponse> concatenatingMove(
    ...
    // media_kit's move inserts *before* the target index, while just_audio
    // expects the item to land AT the target index. When moving downwards
    // (currentIndex > newIndex) inserting before newIndex already lands it
    // at newIndex; when moving upwards the removal shifts subsequent items,
    // so insert before newIndex + 1.
    return _player
        .move(
